...
class _LoanFormState extends State<LoanForm> {
  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      setState(() {
        _ageRestricted = result['ageRestricted'];
        _errorMessage = result['errorMessage'].toString();

        if (_ageRestricted == true) {
          _loanApproved = false;
          _loanAmountResult = 0;
          _loanPeriodResult = 0;
          return;
        ...
        children: [
          ...
          Column(
            children: [
              ...
              const SizedBox(height: 8.0),
                if (_ageRestricted != null)
                Text(
                  _ageRestricted == false ? "Age is accepted." : "Age is either too small or big.",
                  style: TextStyle(
                    color: _ageRestricted == false ? Colors.greenAccent : AppColors.errorColor,
                    fontWeight: FontWeight.bold,
                    fontSize: 16,
                  ),
                ),
                Text(
                  _loanApproved == true ? "We accept the loan amount and period." : "We do not accept this loan amount and period.",
                  style: TextStyle(
                    color: _loanApproved == true ? Colors.greenAccent : AppColors.errorColor,
                    fontWeight: FontWeight.bold,
                    fontSize: 16,
                  ),
                ),
